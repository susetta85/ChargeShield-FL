...
>
> **Addendum (2026-07-24):** the "13,073 sessions" figure used throughout CS1/CS2/CS3 below was the JPL-mislabeled-as-Caltech 2019+2020 file pair specifically. Under the current 3-real-site config (`config/experiment.yaml`, all years 2018-2021 per site, 2019-2021 for Office 1), the combined raw total across all 3 sites is **≈66,713** sessions (Caltech ≈31,404, JPL ≈33,629, Office 1 ≈1,680) — roughly 5× larger than every session count quoted below, not just a relabeling. Any power/sample-size analysis in this document (see also `docs/Roadmap.md`'s note on this) needs re-deriving at the new N before being used in the paper.
>
> **Addendum (2026-08-28) — infrastructure and attack-interface correction, found
> while preparing the DSN 2027 paper draft, not covered by the corrections above.**
> (1) **Topology.** "The network topology comprises 12 nodes distributed across 4
> clusters" (§ above) was never built as a live protocol-level emulation. The
> real, deployed, end-to-end-tested Containerlab/NVFLARE topology
...
> describe the 12-node/4-cluster topology as current infrastructure in the paper.
> (2) **FedMIA.** This document's framing — "applies a federated MIA (FedMIA) to
> quantify information leakage" — describes `src/plugins/attacks/fedmia.py`,
> which is not part of the current attack interface
> (`src/plugins/attacks/__init__.py::ATTACK_REGISTRY` = Yeom/Shadow/LiRA only)
> and is never instantiated in the real experiment pipeline (`ByzantineDetector(...)`
> in `scripts/run_experiments.py` never passes `fedmia=`, so this signal defaults
> to disabled). Every case study result in this document, and every result in
> the current campaign, is produced by the Yeom → Shadow → LiRA hierarchy (LiRA
> primary), not by the `fedmia.py` plugin. Do not cite `fedmia.py` as an active
> module in the paper.
>
> **Addendum (2026-09-04, found during a documentation audit, task #69) — expected-results/AUC
> framing below is stale, and round counts don't match the real protocol.** (1) CS3's "Expected
> Results" table predicts no-DP AUC-ROC 0.60-0.70; the real, corrected finding is the opposite — no
> detectable leakage even without DP (composed AUC-ROC ~0.4992-0.5018 across every configuration,
> Wilcoxon-confirmed, `docs/MetricsReference_DSN2027.md` §8). This hypothesis was tested and
> falsified, not confirmed — read every "Expected Results" table in this document as the project's
> original (pre-experiment) hypothesis, not its finding. (2) Passages that frame an AUC near 0.5
> under DP (e.g. "≈0.5172") as "DP suppressing leakage" are also stale — the corrected result is
> that AUC stays near 0.5 even without DP, so DP is not what's producing that number; there is no
> detected leakage for DP to suppress in the first place. (3) The 100/200/500/1000-round sweep
> designs described throughout (§2.1, CS1/CS2/CS3 experimental setups) were never run at those
> round counts — the real protocol uses 10 FL rounds throughout (`config/experiment.yaml`,
> `fl_rounds`), consistent with every real result cited in `README.md`. Full current findings:
> `docs/DSN2027_Positioning.md`, `docs/MetricsReference_DSN2027.md`.
...
> - **Fixed 2026-07-22 (same-day as flagged, independent review, punto B1) — code-level fix confirmed wired 2026-08-31.** `_clip_weights()` (used by both DP-FedAvg's `privatize()` and central DP's `clip_only()`) previously clipped the L2 norm of the client's full post-training weight vector, not the *delta* from the global model, and did not exclude BatchNorm running-stat buffers from that norm — with `max_grad_norm=1.0` on a ~650-parameter model, this plausibly engaged the clip most rounds, uniformly shrinking the whole model (BN buffers included) back toward zero every round, confounding "DP suppresses MIA" with a clipping artifact rather than the Gaussian noise alone. **Fix**: when a `reference_weights` argument is supplied, `_clip_weights()` now clips `weights - reference` (the actual per-round contribution) and reconstructs `reference + clipped_delta`; BatchNorm buffers are excluded from the norm and passed through unchanged. Re-verified 2026-08-31 that both call sites actually pass `reference_weights` (not just that the fix exists in `_clip_weights()` itself): `run_fl_rounds()` passes `reference_weights=pre_round_weights` to both `gm.clip_only()` and `gm.privatize()`, and the LiRA shadow-model calibration loop passes `reference_weights=_shadow_pretrain_weights` to the matching shadow-side call — so the fix is live on both the real client path and the shadow path used for LiRA calibration, not dead code reachable only via a fallback. **Still open, not done**: the "needs empirical investigation" half of this note — logging how often/how hard the clip actually engages on real data — was never instrumented. `factor` (the clip ratio, computed in `_clip_weights()`) is a local variable, never logged, returned, or exposed anywhere; there is no record in the codebase of clip frequency/magnitude on any real run. The code-level confound (absolute vs. delta clipping) is fixed; the empirical characterization of how much the *pre-fix* absolute clipping was contributing to observed DP-suppression numbers was never measured and cannot now be reconstructed after the fix.
...
>
> **How large is the naive-vs-tighter-composition gap, concretely? (verified 2026-09-14, external fact-check).** Jayaraman & Evans, "Evaluating Differentially Private Machine Learning in Practice" (USENIX Security '19) — verified directly against the paper's own arXiv text and the primary author's blog post (not accepted from a secondhand paraphrase) — measure this gap on a two-layer neural-network classifier: **Rényi DP (RDP) matches naive composition's utility at roughly a 50× tighter budget** (53% accuracy loss at ε=10 under RDP vs. ε=500 for the same accuracy loss under naive composition). Separately, they find that even the *tighter* RDP-trained model does not eliminate leakage: at ε=1000, a white-box membership-inference attacker still achieves **0.399 membership advantage** against the RDP-trained model. Two claims sometimes attached to this paper did **not** verify and are deliberately not cited here: "RDP keeps accuracy loss near zero" (false — the real figure is 53% loss at ε=10), and an 82%-PPV figure attributed to "an adversary observing multiple parallel models/FL rounds" — the actual 82%-PPV result comes from a *different* experiment (retraining the *same* model 5 independent times and checking which records are repeatedly flagged), not from repeated-round observation, so it does not transfer to our composed multi-round LiRA setting without a separate argument we have not made.
>
> **Path to formal DP / tightening the bound we report.** Three options were identified, in increasing order of effort:
> - **(A) Closed-form advanced composition (Dwork & Roth, 2014, Theorem 3.20).** Cheapest: a known closed-form bound, computable directly from `epsilon`/`delta`/`fl_rounds` already logged per run — no new dependency, retroactively computable on existing results. **Implemented 2026-09-14** (task #118, Sprint 10zz+83): `_advanced_composition_epsilon()` in `scripts/run_experiments.py` computes ε' = ε·√(2k·ln(1/δ')) + k·ε·(e^ε−1) for k=`fl_rounds`, wired into every new result as `epsilon_cumulative_advanced`/`delta_cumulative_advanced`/`epsilon_cumulative_best_known` (= min of naive and advanced) alongside `epsilon_cumulative_naive`; `scripts/compute_advanced_composition.py` recomputes it retroactively on every already-completed JSON from the config alone (no rerun needed); `tests/test_advanced_composition.py` (5 tests) verifies the formula independently. **Honest result, not the hoped-for tightening**: at the fl_rounds=10 used throughout this project's campaign, advanced composition is **never tighter than naive** for any of the three ε values used (1.0/0.5/0.1) — verified empirically across all 45 completed DP experiments in the main campaign (0/45 cases where advanced wins) and confirmed analytically: advanced composition is a large-k/small-ε asymptotic improvement whose crossover point (the first round count k at which it beats naive) is k=29 at ε=0.1, k=187 at ε=0.5, and **never** at ε=1.0, because ε≥ln(2)≈0.693 makes advanced composition's dominant linear term (k·ε·(e^ε−1)) grow strictly faster than naive's (k·ε) for any k. Since our campaign uses only 10 rounds, none of our three ε values reaches its crossover (0.1 needs 29, 0.5 needs 187, 1.0 needs none reachable). `epsilon_cumulative_naive` therefore remains the tighter, and correctly reported, bound for every result in this project — option (A) was implemented and honestly found not to help at our scale, which is itself a useful, citable characterization of when it would (many more FL rounds, or ε_per_round below ln 2).
> - **(B) Closed-form RDP accounting for the Gaussian mechanism specifically.** Moderate effort: RDP has a known closed form for the Gaussian mechanism (no general-purpose library needed), and — unlike (A) above — would likely tighten our reported bound regardless of round count, since it is the same class of accounting that gives Jayaraman & Evans their 50× figure. Not yet implemented; now the more promising remaining path.
> - **(C) Full external DP-accounting library** (Opacus, TensorFlow Privacy, or Google's `dp-accounting`), replacing weight perturbation with true DP-SGD per-sample accounting end-to-end. Most rigorous, most engineering effort (new dependency, per-sample gradient computation, likely a new experimental campaign) — left as future work given the DSN 2027 submission timeline.
>
> (A) is implemented (2026-09-14) but does not tighten the reported bound at our scale; (B) and (C) remain future work. `epsilon_cumulative_naive` remains the tightest bound this codebase actually reports for every result to date.
>
...
> - **DP-FedAvg**: the server receives each client's raw update and immediately clips + noises it **per client**, before combining. This is what `GradientManager.privatize()` implements (`run_fl_rounds()`: `private_update = gm.privatize(update, ...)` inside the per-client loop, then `agg.collect(private_update)`). This per-client, noise-before-aggregation placement is a stricter, non-standard variant — it is **not** literally what McMahan et al. (2018)'s actual DP-FedAvg Algorithm 1 describes (client-side clip, single server-side aggregate noise draw); see **Central DP** below for the mode that mechanism actually corresponds to, and `docs/ReadingList_DSN2027.md`'s 2026-08-14 entry for the full analysis.
>
...
>
> **Local DP run 2026-08-30 (`local-sweep1`/`local-sweep2`) — prediction confirmed, but read the caveat below before citing it as a security guarantee.** All three attacks land at chance (AUC≈0.499–0.50, no IDS alerts) — matching the prediction. **Important limitation, found 2026-08-31 while verifying a related claim**: in the *simulation* (`run_fl_rounds()`, single Python process, no real client/server boundary), `dp_mode="local"` does **not** implement an access-control boundary — it implements *report suppression*. The noise is computed via the same `gm.privatize()` call used by `dp-fedavg`, and the only difference under `local` is that `raw_updates`/`raw_global_weights` are set to `None` before being written into the saved `results` dict, so nothing downstream (LiRA, the IDS, the exported JSON) can read them. But the raw, pre-noise update still exists in memory and would still be returned if the `FLArtifactCollector` were queried directly for that round — there is no real barrier preventing it, only the fact that nothing in the pipeline currently asks. No test in the codebase actively attempts to read the raw update under `dp_mode="local"` to confirm it is inaccessible; the one test with a name suggesting this (`test_local_dp_hides_raw_updates_from_ids`) only asserts that a Python dict field is `None`, and has never actually been executed in this sandbox (`torch` unavailable). **The real client-side enforcement exists only in the NVFLARE deployment path** (`chargeshield_executor.py`: `gm.privatize()` runs before `outgoing_dxo.to_shareable()`, i.e. before the update ever leaves the client process) — but that path itself has not yet been run end-to-end with `dp_mode="local"` (Sprint 10aaa, README, 2026-08-31: the NVFLARE ML-Plane wiring across all three `dp_mode`s is unverified by a real execution). So: the AUC≈0.5 result above is real and correctly measured, but it should not yet be cited as empirical confirmation that raw updates are *architecturally unreachable* under local DP — only that this pipeline's attacks don't currently look for them there.
>
> **Updated 2026-07-24 — Central DP now run for real, both predictions confirmed.** `make experiment-central-dp` was run at `ε=1.0` and `ε=0.1` (single seed=42 each, `n_shadow=16`, 10 rounds): LiRA mean AUC-ROC came back **0.7430** at ε=1.0 and **0.8118** at ε=0.1 (`experiments/experiment_20260724_111109.json`/`experiment_20260724_144952.json`), while Yeom/Shadow both stayed at ≈0.499 (chance level) in both runs — exactly the predicted pattern (LiRA unsuppressed, Yeom/Shadow suppressed) and, if anything, a sharper result than anticipated: the *tighter* nominal budget (ε=0.1) produced *higher* measured LiRA leakage, not lower, because central DP's aggregate noise only disrupts the shared warm-start model, never the per-client update LiRA attacks (see `docs/PrivacyExposureScore_v1.md` for the full reading and the composite PES_v1 score computed from these numbers: 0.243 and 0.567 respectively — the largest PES value the project has produced). **Local DP has not yet been run** — that prediction (all three attacks degrade, IDS alerts increase) remains unconfirmed empirically. **Caveat**: both Central DP numbers above are single-seed; the project's own 5-seed standard (already applied to every DP-FedAvg sweep point) has not yet been applied to Central DP, so neither number should be treated as final until multi-seed confirmation lands.
>
> **Superseded (2026-08-26 — see README Sprint 10x through 10dd).** Multi-seed confirmation did eventually land (`central-sweep1`/`central-sweep2`, 2026-08-04/08-10: mean LiRA AUC 0.7147 at ε=1.0, 0.8334 at ε=0.1, both bootstrap 95% CIs excluding 0.5) — but a six-fix investigation through August (cross-cluster pooling contamination, asymmetric σ floor, uncalibrated-outlier scoring, a μ_in fallback-formula artifact, and finally a structural bug where the shadow-model sampling universe could never include non-members at all, making real IN-calibration for non-members architecturally impossible) found that **every one of these Central DP numbers, including the multi-seed CIs, was substantially an artifact of the LiRA implementation, not real leakage.** With the full fix chain applied, a four-way convergent re-verification (no-DP 10-round, Central DP ε=1.0 10-round, the real ContainerLab/NVFLARE deployment, and dp-fedavg ε=8/16 checks) shows **no detectable LiRA leakage in any configuration** (composed AUC 0.4992–0.5018 across all four). **Do not cite the 0.7430/0.8118/0.7147/0.8334 figures above in the paper** — they are kept here only as a documented, dated record of what the pre-fix pipeline reported and why it was wrong. See README Sprint 10dd for the exact corrected numbers and `docs/DSN2027_Positioning.md` for the revised thesis.
...
> - `nvflare/project.yml` provisions PKI/network participants only — there is no NVFLARE job/app (no `Executor`/`Controller` referencing `AutoencoderTrainer`, `GradientManager`, `PrivacyAuditor`, or `ByzantineDetector`), so even a successful `make deploy` would bring up empty containers with nothing to run.
...
>
> **Started 2026-07-22, first real execution 2026-07-24** — see `docs/NVFlareIntegration.md` for the current status: the job (`nvflare/jobs/chargeshield_poc/`) has a real client-side Executor wrapping `AutoencoderTrainer` and a custom server-side `ChargeShieldAggregator` wrapping the same `FedAvgAggregator`/`PrivacyAuditor`/`ByzantineDetector`/`GradientManager` classes the simulation uses (not NVFLARE's built-in aggregator — that was superseded fase 2, 2026-07-22). Written and reasoned through manually in an environment without `torch`/`nvflare` installed, then run for real for the first time on 2026-07-24 (`make nvflare-sim-smoke`, nvflare 2.8.1, on the user's own machine) — it crashed, and 3 real bugs were found and fixed (a project-root path-resolution bug, a partial-initialization-state bug in the aggregator, and a stale timezone-unaware `hour_of_day` calculation that silently diverged from the simulation's fixed version). Of the original 7 `VERIFY:` assumptions marked in the code, 5 have since been confirmed correct against NVFLARE's actual public source (not just reasoned) — 2 remain genuinely open (local round-number counters vs. NVFLARE's authoritative round). A fully successful multi-round re-run is still pending confirmation as of this writing. This does not change the multi-week Containerlab estimate above; it gives that estimate real, executed evidence instead of only a reasoned starting point.
