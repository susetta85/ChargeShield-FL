...
>
> **Path to formal DP / tightening the bound we report.** Three options exist, in increasing order of effort:
> - **(A) Closed-form advanced composition (Dwork & Roth, 2014).** Cheapest: a known closed-form bound tighter than naive composition (though looser than RDP), computable directly from `epsilon`/`delta`/`fl_rounds` already logged per run — no new dependency, retroactively computable on existing results. Not yet implemented.
> - **(B) Closed-form RDP accounting for the Gaussian mechanism specifically.** Moderate effort: RDP has a known closed form for the Gaussian mechanism (no general-purpose library needed), and would let us report a bound in the same ballpark as the 50× figure verified above. Not yet implemented.
...
>
> None of (A)/(B)/(C) is implemented today; `epsilon_cumulative_naive` (below) remains the only composition figure this codebase computes.
>
