...
Federated Learning (FL) deployed in operational technology (OT) environments such as electric vehicle (EV) charging infrastructure introduces a category of network traffic — gradient uploads and aggregation signals — that is architecturally invisible to existing OT security frameworks. The Purdue Model, IEC 62443, and ISA-99, which govern the communication architecture of industrial control systems, define well-structured hierarchical communication planes for field bus, supervisory, and enterprise traffic. None of these standards provide a principled abstraction for machine learning gradient traffic, which crosses Purdue levels L0 through L3 in a manner that is structurally distinct from, yet entangled with, standard SCADA and charging protocol traffic — the OCPP 1.6/2.0.1 and MQTT v5 protocol layer referenced throughout this document was a design proposal that was never built as a live emulation (see correction notice above); the real, deployed topology has no per-node OT protocol endpoint.

This document introduces and formally describes the **ML Plane**: a transversal logical observation layer that intercepts, annotates, and routes ML-specific events across all Purdue levels without modifying the underlying FL training or aggregation logic. The ML Plane is not a new Purdue level; it is an observation plane analogous to a monitoring plane in software-defined networking, enabling both attack/auditing components (currently: the Privacy Auditor and the registered Yeom/Shadow/LiRA attacks — the `FedMIA` plugin described in places below as a live consumer is dead code, never instantiated in the real pipeline) and defense components (ByzantineDetector) to operate on FL gradient traffic that would otherwise appear as generic encrypted data indistinguishable from standard operational traffic.
...
- **Encoder:** Input layer (6 features) → Dense(16) → BatchNorm1d(16) → ReLU → Dense(8) → BatchNorm1d(8) → ReLU → Dense(4) [bottleneck, **no activation**]
- **Decoder:** Dense(8) → ReLU → Dense(16) → ReLU → Output layer (6 features, **Sigmoid**)

(Corretto 2026-09-09 — la versione precedente di questa sezione descriveva erroneamente il bottleneck come "Dense(4, ReLU)" e l'output come "linear activation": il codice reale, `src/core/autoencoder.py::Encoder`/`Decoder`, non applica alcuna attivazione al layer del bottleneck — di proposito, per non comprimere lo spazio latente in [0,+∞) e dimezzarne la capacità espressiva — e usa Sigmoid, non lineare, sull'output, perché le feature sono normalizzate in [0,1]. Mancava anche la menzione di `BatchNorm1d` dopo ciascuno dei due layer nascosti dell'encoder, presente nel codice per stabilizzare il training quando i dati locali dei client hanno distribuzioni molto diverse tra loro — rilevante in un contesto FL con siti reali di dimensione molto sbilanciata come Office1 vs Caltech/JPL.)
...
**Activations (corretto 2026-09-09 insieme alla sezione Architecture sopra):** ReLU per i quattro layer nascosti (due nell'encoder, due nel decoder); **nessuna attivazione** sul bottleneck (per preservarne il segno e la piena capacità espressiva, non comprimerlo in [0,+∞)); **Sigmoid**, non lineare, sull'output — coerente con le feature normalizzate in [0,1]. ReLU è scelta per i layer nascosti (non per l'output, come erroneamente affermato in precedenza) perché: (a) non satura per input positivi grandi, rilevante per `total_energy_kwh` e `max_power_kw` che possono assumere valori grandi dopo la normalizzazione; (b) produce attivazioni più sparse, incoraggiando il bottleneck a usare attivamente un sottoinsieme delle sue 4 dimensioni; (c) il suo gradiente è 1 per input positivi e 0 per negativi, il che interagisce bene con il clipping DP (i gradienti clippati attraverso layer ReLU mantengono l'informazione di segno).

**Regolarizzazione — nessun dropout.** L'architettura non include layer `Dropout` in nessun punto (encoder o decoder) — l'unica regolarizzazione presente è `BatchNorm1d` sui due layer nascosti dell'encoder (vedi sopra), il cui scopo primario è stabilizzare il training in presenza di distribuzioni locali molto diverse tra i client (non prevenire l'overfitting di per sé). La difesa principale contro la fuga di informazione per membership inference in questo progetto è il rumore Differential Privacy (`GradientManager`, tre placement: dp-fedavg/central/local — vedi `docs/PrivacyAuditor.md`), non tecniche di regolarizzazione come il dropout. Questa è una scelta di design esplicita, non un'omissione: con solo 6 feature di input e 570 parametri totali nell'architettura di default, il rischio di overfitting è già limitato dalla capacità ridotta del modello, e l'aggiunta di dropout non è stata necessaria per ottenere risultati stabili nella campagna sperimentale. Va comunque annotato esplicitamente come scelta dichiarata (non testata in alternativa) se il paper discute le difese contro la MIA.
...
where `K` is the number of participating clients, `n_k` is the dataset size of client `k`, and `g_k` is the (potentially DP-noised) gradient submission from client `k`. In the currently deployed ChargeShield-FL, `K = 3` (the real ACN-Data site clients: Caltech, JPL, Office 1; two additional synthetic clients are used only in the separate IDS/Byzantine validation sweep, never for privacy measurements), and dataset sizes reflect the real, unequal distribution of ACN-Data sessions across sites (the "12 nodes across 4 clusters" figure below reflects the abandoned Sprint 5 design — see correction notices above).
...
ChargeShield-FL Scenario 1 assumes an **honest-but-curious aggregator**: the aggregator at L3 follows the FL protocol faithfully (computes correct weighted averages, distributes correct global models) but attempts to infer membership of individual training samples from the gradient submissions it receives. Under client-side DP with the Gaussian Mechanism, the aggregator observes only the DP-noised, clipped gradient (under `dp_mode="local"`; under `dp_mode="central"` — the placement that actually matches McMahan et al.'s DP-FedAvg construction — the server itself adds the noise to the aggregate). The (epsilon, delta)-DP guarantee bounds the aggregator's ability to distinguish the gradient computed on a dataset containing a target record from the gradient computed on a dataset without that record. The registered attacks' AUC-ROC metric (Yeom/Shadow/LiRA — not the dead-code `FedMIA` plugin) operationalizes this bound empirically: if DP is effective, AUC-ROC should approach 0.5 (random guessing); if DP is insufficient (epsilon too large, or DP disabled), AUC-ROC should be significantly above 0.5. As of the completed 5-seed campaign (see `docs/MetricsReference_DSN2027.md` §8), measured AUC-ROC is consistent with chance across every tested configuration — no configuration shows statistically significant membership-inference leakage.
