...
| Model inversion / Deep Leakage from Gradients (Zhu et al. 2019, DLG) attacks (recovering raw session records from a client's submitted update) | Distinct from MIA (reconstruction, not membership). **Correction (2026-09-11, user question — the previous version of this row over-simplified the defense):** the primary reason DLG's literal construction does not apply as-is is architectural, not dimensional — see the row below. The tabular/continuous nature of the 6 EV features (correlated, no spatial redundancy or pixel-smoothness prior for an optimizer to exploit, unlike images) makes any eventual reconstruction attempt harder to interpret and validate than in the image domain, but this is a claim about attack *difficulty*, not *infeasibility*: it has not been empirically tested against this autoencoder, and the model's small size (570 parameters) is exactly the regime the gradient-inversion literature considers *easier* to invert, all else equal — so this project does not claim tabular-domain immunity, only that the question requires the dedicated attack module below, not an assumption. | Future work |
| Gradient inversion attacks (Zhu et al. 2019 and follow-ups, e.g. Geiping et al. 2020) | **Corrected reasoning (2026-09-11):** the earlier version of this row attributed infeasibility to "batch size 32+" alone — an incomplete rationale. The primary architectural mitigation is that ChargeShield-FL's FL clients never transmit a raw per-batch gradient at all: `ChargeShieldExecutor`/`run_fl_rounds()` return the client's full weight vector (`get_weights()`/`state_dict()`) *after* `local_epochs` (50 by default, `config/experiment.yaml`) of local SGD over multiple batches (`batch_size=32`) drawn from the client's entire local dataset — i.e. the aggregate effect of hundreds of SGD steps collapsed into one update, not the single closed-form gradient DLG's optimization-based matching loss is built to invert. This "many local steps aggregated into one FedAvg-style update" defense is well known in the FL privacy literature (the same reason McMahan et al.'s original FedAvg construction is considered harder to attack via DLG than a naive single-step gradient-averaging scheme) and is architecturally present in every DP mode, including no-DP — it does not by itself provide a formal guarantee, and larger local_epochs values used elsewhere in this project (e.g. 1000, for some canary positive-control configs) strengthen rather than weaken it. Batch size 32 is a secondary, reinforcing factor, not the primary one as previously stated. Not formally evaluated in this paper — a dedicated Gradient Inversion attack module is listed as future work (§10). | Future work |
