...
ChargeShield-FL is a modular, reproducible research framework for evaluating Membership Inference Attacks (MIA) against Federated Learning (FL) systems deployed within Electric Vehicle (EV) charging infrastructure. The framework does not propose a novel FL algorithm; rather, its scientific contribution is the framework itself: a realistic, privacy-aware testbed that bridges the gap between academic FL privacy research and the operational realities of real-world EV charging data. **As built and currently deployed** (see the correction notices above), ChargeShield-FL trains an unsupervised autoencoder across the **three real ACN-Data sites — Caltech, JPL, and Office 1** — as FL clients, orchestrating a complete FL training lifecycle using NVFLARE 2.7.2 (single-process simulator and a genuine 5-node Containerlab/NVFLARE deployment: `server`, `caltech`, `jpl`, `office1`, `fl-admin`). It implements Differential Privacy (DP) via the Gaussian Mechanism in three wired placements (`dp-fedavg`, `central`, `local`) and evaluates membership leakage using a registry of attacks — Yeom et al. (2018), Shadow MIA, and LiRA (Carlini et al., 2022, the primary/strongest attack) — measuring the trade-off between privacy budget ε and attack success expressed as AUC-ROC. The twelve-node/four-cluster OCPP-1.6/OCPP-2.0.1/MQTT-v5 protocol topology and the FedMIA shadow-model attack described in the body of this document below were an early design that was never built as a live emulation and is not part of the current attack interface; both are retained below as historical/future-extension material, not as current fact. The framework is containerized via Docker and Containerlab, secured with mTLS (NVFLARE-provisioned PKI; the WireGuard overlay described below was part of the abandoned 12-node design and is not part of the current deployment), and structured around four software design patterns that together enable modular extensibility without modification of core components. ChargeShield-FL is intended as a community testbed for reproducible privacy research using real EV charging data, addressing an understudied intersection of federated learning and differential privacy.
...
- A **privacy evaluation pipeline** that trains an unsupervised autoencoder on real ACN-Data sessions, applies Gaussian Mechanism DP (in one of three wired placements: `dp-fedavg`, `central`, `local`), and subsequently evaluates membership leakage through the registered attack suite (`ATTACK_REGISTRY` = Yeom/Shadow/LiRA, dispatched via `run_registered_attacks()` in `scripts/run_experiments.py`) — LiRA (Carlini et al., 2022) is the primary attack; the older `FedMIA` shadow-model plugin (`src/plugins/attacks/fedmia.py`) referenced elsewhere in this document is dead code, never wired into `ByzantineDetector` or the pipeline (see correction notices above).
...
The autoencoder architecture (6→16→8→4→8→16→6, 570 trainable parameters — see `src/core/autoencoder.py`, confirmed consistent with `docs/MLPlane.md` and the paper skeleton's §5) is intentionally sized to be trainable on constrained edge hardware. At 32-bit float precision, model weights occupy approximately 2.3 KB. A local training batch of 256 sessions requires approximately 1.2 MB of activation memory during the forward pass. Gradient buffers add a similar footprint. This is compatible with the Raspberry Pi CM4 (4 GB RAM) and Intel NUC configurations. On bare EVSE hardware (512 MB RAM, typically 200–300 MB consumed by OS and firmware), this remains infeasible — not because the model is large, but because the gradient computation machinery (automatic differentiation graph, optimizer state) requires memory that is unavailable given the firmware's working set.
...
> **Implementation status (updated 2026-07-24).** The plugin mechanism described below **is now implemented**, but as an explicit registry, not filesystem-based discovery: `src/core/base_attack.py` defines the `BaseAttack` ABC (`run(cfg, train_sessions, holdout_sessions, fl_results, **kwargs) -> dict[int, dict[str, Any]]`), and `src/plugins/attacks/__init__.py` maps attack names to classes in `ATTACK_REGISTRY: dict[str, type[BaseAttack]]`. `YeomAttack`, `ShadowAttack`, and `LiRAAttack` (`src/plugins/attacks/yeom.py`/`shadow.py`/`lira.py`) are thin `BaseAttack` subclasses that lazily import and delegate to the original, unmodified `run_fedmia()`/`run_fedmia_shadow()`/`run_lira()` functions in `scripts/run_experiments.py`. `run_registered_attacks()` (also in `run_experiments.py`) iterates `ATTACK_REGISTRY.items()` and is the single dispatch point used by both `run_experiments.py::main()` and `scripts/run_nvflare_mia.py::main()` — adding a new attack requires only a new module + one line in `ATTACK_REGISTRY`, no change to the dispatcher. `FedMIA` (`src/plugins/attacks/fedmia.py`, discussed in §6.5) is separate, older, dead code — it does **not** implement `BaseAttack` and is not in `ATTACK_REGISTRY`.

**Problem Statement.** MIA research is a rapidly evolving field. Yeom/Shadow/LiRA are the three implemented attacks, but the framework must accommodate future attacks — gradient inversion [Geiping et al., 2020], property inference [Ateniese et al., 2015], attribute inference [Zhao et al., 2021] — without requiring modification of the evaluation harness.

**Design intent, as built.** New attack modules are placed in `src/plugins/attacks/` implementing `BaseAttack.run()`, then added to `ATTACK_REGISTRY` by name → class. This was chosen over filesystem-based auto-discovery (originally sketched below) for two reasons: an explicit dict avoids executing arbitrary code found on disk (a real concern for a "community-contributed attacks" framework), and with three attacks today, central registration costs one line and stays easy to audit. Filesystem scanning remains a possible future upgrade if/when external contributions arrive, but is not required for the registry pattern itself to work or to support the "reusable benchmark/framework" framing (§1, DSN2027_Positioning.md).

**Why Not a Subclass Hierarchy Without a Registry.** A bare subclass hierarchy still requires *something* to know which concrete classes exist. `ATTACK_REGISTRY` is that something, kept in one file (`src/plugins/attacks/__init__.py`) instead of scattered import statements inside the evaluation harness.

**Why Not (Yet) Filesystem-Based Discovery.** Auto-scanning `attacks/` for conforming classes would let external collaborators add attacks with zero registry edits, but also means running whatever code lands in that directory — an explicit registry was judged the safer default while the project has a small, internally-authored set of attacks.
...
The topology described immediately below (one NVFLARE server container, twelve NVFLARE client containers, four cluster-internal Docker bridge networks) was the original Sprint 5 design and was never built as a live emulation (see correction notices above). **The real, deployed, end-to-end-tested topology** (`containerlab/topology.clab.yml`, re-verified 2026-09-09) is a **5-node star**: one `server` container (running `ChargeShieldAggregator` — FedAvg + PrivacyAuditor + ByzantineDetector, all server-side, no separate auditor/IDS/broker containers), three real-site FL client containers (`caltech`, `jpl`, `office1`), and one `fl-admin` console — all linked point-to-point to `server`, with no OT/OCPP/MQTT link and no per-cluster network segmentation.
...
No publicly available framework exists that evaluates MIA against FL using real, multi-site EV charging data end to end. Existing MIA evaluations against FL [Nasr et al., 2019; Carlini et al., 2022] use generic FL settings (image classification, tabular synthetic data) without real EV operational data or hardware-realistic computational boundaries. Existing EV infrastructure security work [Alcaraz and Lopez, 2022; Acharya et al., 2020] does not include a FL privacy evaluation component. ChargeShield-FL is the first framework to combine: (a) real ACN-Data charging sessions from three genuine sites (Caltech, JPL, Office 1); (b) Gaussian Mechanism DP applied under three distinct wired placements (`dp-fedavg`, `central`, `local`); and (c) quantitative membership inference evaluation via a registered attack suite (Yeom 2018, Shadow MIA, LiRA — primary) — in a single, containerized, reproducible artifact with documented provisioning, deployable both as a single-process simulator and as a genuine multi-container NVFLARE/Containerlab federation. (The multi-protocol OCPP/MQTT simulation and the FedMIA shadow-model attack originally envisioned here were never built as a live emulation — see correction notices above — and are not part of this contribution as currently realized.)

**Contribution 2: The ML Plane Abstraction + the Privacy Auditor (updated 2026-07-31 — the
project's actual central contribution, per explicit user framing correction).**
...
The concrete software realization that makes this more than a conceptual diagram is the **Privacy Auditor** (`src/auditor/privacy_auditor.py`): a domain-agnostic, config-driven (`config/auditor.yaml`) component that consumes exactly the model-update data the ML Plane's `FLArtifactCollector` assembles each round, and computes a gradient-sensitivity-based MIA-risk score and cumulative DP-budget consumption per node, independent of which benchmark attack (if any) is also plugged in. It is the always-on, real-time counterpart to the offline, post-hoc attack evaluation (§6, §9.3's Contribution 3 below): the ML Plane is the substrate, the Privacy Auditor is the first real consumer built specifically to reason about privacy risk from that substrate, and FedMIA/Yeom/Shadow/LiRA are a separate, swappable case-study layer used to validate that the risk the Auditor flags is real, measurable leakage rather than a false alarm — not the contribution in their own right (§9.2 already states this for FedMIA specifically; it now applies to the whole attack suite). **Current wiring caveat:** `PrivacyAuditor.audit()` is invoked imperatively (once per node per round, from `run_ids()` in simulation and the equivalent logic in `ChargeShieldAggregator` for NVFLARE) rather than via a literal `MLPlaneListener` subscription like `FLArtifactCollector` uses — the data dependency on the ML Plane is real today, the event-driven *activation* is not yet; see `docs/DSN2027_Positioning.md`'s "The actual contribution" section for the full accuracy note and the scoped follow-up this implies.
